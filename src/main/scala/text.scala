...
object analyze {
  ...
  def tokenizeLucene(s: String): List[String] = {
  ...
  def labelTokens(labels: Map[String, String]): Map[String, List[String]] = {
    for {
      (key, label) <- labels
      tkns = tokenizeLucene(label)
      if tkns.size > 0
    } yield (key, tkns)
  }
  ...
  case class ExtTokenizerSs(ignoreCase: Boolean = true) extends Tokenizer {
    ...
    def tokenize(input: String): Array[Token] = {
      tokenizeLucene(input) map intern toArray
    }
...
//  labelTokens(dbpediaLabels)
  ...
  tfidf3.tokenIterator.asScala.toList map { t =>
